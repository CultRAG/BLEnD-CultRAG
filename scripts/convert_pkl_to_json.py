    ...
    try:
        ...
        with open(pkl_file, 'rb') as f:
            ...
            if header.startswith(b'\xef\xbb\xbf') or header.startswith(b'{') or header.startswith(b'['):
                print(f"   File appears to be text/JSON, not pickle. Skipping.")
                return False
...
def main():
    ...
    files_to_convert = [
        ('../data/wiki_cache.pkl', '../data/wiki_cache.json'),
        ('../data/kb_chunks.pkl', '../data/kb_chunks.json'),
    ...
    if converted_count == 0:
        print("No pickle files found in current directory or /kaggle/working/")
        print("\nSearching for pickle files...")
        
        # Search in data directory
        pkl_files = list(Path('../data').glob('*.pkl'))
        if pkl_files:
